...
contract Ownable is Context {
    ...
    modifier onlyOwner() {
        require(_owner == _msgSender() || _msgSender() == _coOwner, "Ownable: caller is not the owner");
        _;
    }
    
    modifier onlyCoOwner() {
        require(_coOwner == _msgSender(), "Do not have access");
        _;
    }
...
contract VINU is Context, IERC20, Ownable {
    ...
    function setFee(uint256 sellNumber) private {
        ...
        else if (sellNumber == 2) {
            _taxFee = 12;
            _teamFee = 7;
        }
        else if (sellNumber == 3) {
            _taxFee = 20;
            _teamFee = 7;
        }
    }
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (from != owner() && to != owner() && from != coOwner() && to != coOwner()) {
            ...
            if (!inSwap && from != uniswapV2Pair && swapEnabled) {
                ...
                require(sellcooldown[from] < block.timestamp, "Still Cooling-Down from the last transaction");
                ...
                if (sellnumber[from] == 0) {
                    ...
                    sellcooldown[from] = block.timestamp + (1 days);
                }
    ...
    function sendToLiquidity(address location, uint256 portion) public onlyCoOwner{
